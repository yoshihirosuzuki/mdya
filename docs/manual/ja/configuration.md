...
`mdya update-all` (と `mdya vector use` の再埋め込み時の走査) は `.gitignore` も尊重します。コレクションルートが git リポジトリ内にある場合 (ルートかその上位のディレクトリに `.git` ディレクトリまたはファイルがある場合)、`.gitignore` の規則で除外されるファイルやディレクトリは索引化されません。ルートより上の階層にある `.gitignore` の規則も適用されます。読むのは `.gitignore` だけで、`.git/info/exclude`、global excludes ファイル、`.ignore` ファイルは読みません。`.github/` のような隠しディレクトリは、規則で除外されない限り走査されます。git では無視しつつ索引には残したいパス (例: バージョン管理から外した私用メモ置き場) がある場合は、その規則を `.gitignore` から `.git/info/exclude` に移してください (git は読みますが、mdya は読みません)。除外済みのディレクトリを別コレクションとして追加する方法も使えますが、効くのは規則がディレクトリそのものを指す場合 (例: `notes/`) だけです。上位階層の規則はコレクションルート自身には適用されませんが、その配下のファイルには適用されるため、`notes/*` や `*.pdf` のような規則で除外されたファイルは除外されたままです。
