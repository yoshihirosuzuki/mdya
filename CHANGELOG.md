...
- Bumped `lancedb` 0.30.0 → 0.37.1 and `lance-index` 7.0.0 → 10.0.0 in lockstep. lancedb 0.37 requires the whole `lance-*` ecosystem at one exact version, so neither pin can move on its own. `arrow-array` / `arrow-schema` stay on 58, which both new versions still require. The bump also drops `pprof` → `inferno` → `quick-xml` from the dependency tree, because upstream moved `lance-testing` to dev-dependencies; that chain is what had been holding the bump back.
